...
QR codes use Reed-Solomon (RS) error correction. The black and white pixels represent data, consisting of `k` message symbols and `r` derived error correction symbols. Changing the pixels requires changing the message symbols, but changing the message defeats the purpose of a QR code. Fortunately for us, if a message of length `m` does not use all `k` symbols, `k - m = p` arbitrary padding symbols are added.
...
Here are the earliest descriptions of similar ideas I found online, although likely independently identified many times.
