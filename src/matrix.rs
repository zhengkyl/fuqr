...
impl Matrix {
    pub fn new(data: Data, mask: Option<Mask>) -> Self {
        ...
        matrix.value = vec![Module::Unset; matrix.width() * matrix.width()];
...
pub trait QrMatrix {
    ...
    fn iterate_finder(&mut self, f: fn(&mut Self, usize, usize, Module)) {
        ...
        for i in 0..8 {
            f(self, i, 7, Module::SeparatorOFF);
        }
        for i in 0..7 {
            f(self, 7, i, Module::SeparatorOFF);
        }

        self.iterate_finder_part(0, self.width() - 7, f);
        for i in 0..8 {
            f(self, i, self.width() - 8, Module::SeparatorOFF);
        }
        for i in 0..7 {
            f(self, 7, self.width() - 1 - i, Module::SeparatorOFF);
        }

        self.iterate_finder_part(self.width() - 7, 0, f);
        for i in 0..8 {
            f(self, self.width() - 1 - i, 7, Module::SeparatorOFF);
        }
        for i in 0..7 {
            f(self, self.width() - 8, i, Module::SeparatorOFF);
        }
    }
    ...
    fn iterate_data(&mut self, data: Vec<u8>, f: fn(&mut Self, usize, usize, Module)) {
        ...
        loop {
            ...
            row_dir *= -1;
            row_limit = (row_limit as isize + (self.width() - 1) as isize * row_dir) as usize;
        }
    }
    ...
    fn apply_mask(&mut self, mask: Mask) {
        ...
        for i in 0..self.width() {
            for j in 0..self.width() {
                ...
                self.set(i, j, Module::DataOFF | (module ^ mask_bit(j, i) as u8));
            }
        }
    }
}
